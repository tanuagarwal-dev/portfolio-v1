...
const Card = ({ data }: { data: CardProps }) => {
  ...
  return (
    <StyledWrapper>
      <div
        className="card lg:w-100 h-64 md:w-100 w-74 relative overflow-visible  "
        onClick={() => setIsFlipped(!isFlipped)}
      >
        <div
          className={`content w-full h-full rounded-[10px] ${isFlipped ? 'rotate' : ' '}`}
        >
          <div className="back rounded-[10px] dark:bg-[#151515] w-full h-full absolute flex justify-center items-center overflow-hidden bg-emerald-700">
            <div className="back-content text-white dark:bg-[#151515] bg-emerald-700 dark:text-white absolute w-[99%] h-[99%] rounded-sm flex flex-col justify-center items-center gap-8">
              <svg
                stroke="#ffffff"
